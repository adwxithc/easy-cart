...
const adminDashboard = asyncErrorHandler(async (req, res, next) => {
    ...
    const todaysTOtalDelivery =
        await adminHelpers.calculateTodaysTotalDelivery();
...
const blockOrUnblockUser = asyncErrorHandler(async (req, res, next) => {
    ...
    if (userData) {
        if (userData.status) {
            ...
            const updation = await User.updateOne(
                { _id: id },
                { $set: { status: true, lastModified: Date.now() } }
            );
            if (updation) {
                res.json({
                    message: `${
...
const searchUser = asyncErrorHandler(async (req, res, next) => {
    const users = await User.find({
        fname: { $regex: new RegExp(`^${req.query.key}`, 'i') },
    });
...
const changeProductStatus = asyncErrorHandler(async (req, res, next) => {
    ...
    if (productData) {
        if (productData.status) {
            ...
            if (updated) {
                res.json({
                    message: 'Product has inactivated',
                    status: 'inactivated',
                });
            } else {
...
const updateProductInfo = asyncErrorHandler(async (req, res, next) => {
    ...
    if (updateProduct) {
        ...
        const public_ids = productData.images
            .map((url) => {
...
const listOrUnlistCategory = asyncErrorHandler(async (req, res, next) => {
    ...
    if (categoryData) {
        if (categoryData.status) {
            const statusUpdate = await Category.updateOne(
                { _id: categoryId },
                {
                    $set: {
                        status: false,
...
const listUnlistBrand = asyncErrorHandler(async (req, res, next) => {
    ...
    if (brand) {
        const message = brand.status
            ? 'The brand has been unlisted'
        ...
        if (updated) {
            ...
            const err = new CustomError(
                'Unable to change the status of the brand',
                500
            );
            next(err);
        }
...
const updateBrand = asyncErrorHandler(async (req, res, next) => {
    ...
    if (updated) {
        ...
        const parts = req.originalbrandData.logo.split('/');
        const public_id = parts.at(-2) + '/' + parts.at(-1).split('.')[0];
    ...
    if (updatedOrder) {
        if (req.body.newStatus == 'Canceled') {
            ...
            if (paymentStatus == 'received' && paymentMethod !== 'COD') {
                ...
                await adminHelpers.addMoneyToWallet(
                    req.body.orderId,
                    amount,
                    transactionId,
                    'Order refunded due to admin cancelation'
                );
            }
        }
        res.json({
...
const updateReturnStatus = asyncErrorHandler(async (req, res, next) => {
    ...
    if (newReturnStatus == 'returned') {
        ...
        let productPrice =
            order.items[productIndex].quantity *
            order.items[productIndex].price;
        ...
        if (order.items[productIndex].eligibleForReturn) {
            ...
            if (discountPercentage > 0) {
                ...
                let totalAmount =
                    totalPidAmount / (1 - discountPercentage / 100);
                let totalAmountAfterReturn = totalAmount - productPrice;
                let newDiscountedTotal =
                    totalAmountAfterReturn -
                    (totalAmountAfterReturn * discountPercentage) / 100;

                //RETURNING REMAING AMOUNT AFTER MINUSING NEW DISCOUNTED TOTAL FROM PAIED AMOUNT
                refundAmount =
                    totalPidAmount - newDiscountedTotal > 0
                        ? totalPidAmount - newDiscountedTotal
                        : 0;
            } else {
            ...
            let totalAmountAfterReturn = totalAmount - productPrice;
            refundAmount =
                totalPidAmount - totalAmountAfterReturn > 0
                    ? totalPidAmount - totalAmountAfterReturn
                    : 0;
    ...
    if (updated) {
        await Product.updateOne(
            { _id: productId },
            { $inc: { stock: order.items[productIndex].quantity } }
        );
        res.json({
            success: true,
        ...
        const err = new CustomError(
            'Product return status updation failed',
            500
        );
        next(err);
    }
